...
from src.SlimUNETRpp.SlimUNETRpp import SlimUNETRpp
...
def train_one_epoch(
    ...
    for i, image_batch in enumerate(train_loader):
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.trainer.num_epochs}] Training [{i + 1}/{len(train_loader)}] Loss: {total_loss:1.5f} {log} Compute Time: {compute_time:.5f}s",
            flush=True,
        )
        step += 1
...
def val_one_epoch(
    ...
    for i, image_batch in enumerate(val_loader):
        start_compute_time = time.time()  # 开始计算时间计时
        ...
        for metric_name in metrics:
            metrics[metric_name](y_pred=val_outputs, y=image_batch["label"])
        compute_time = time.time() - start_compute_time  # 计算模型验证时间
        accelerator.log(
            {
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.trainer.num_epochs}] Validation [{i + 1}/{len(val_loader)}] Loss: {total_loss:1.5f} {log} Compute Time: {compute_time:.5f}s",
            flush=True,
        )
        step += 1
    ...
    accelerator.log(metric, step=epoch)
    trains_epochs.append(epoch + 1)
    dices_avg.append(metric["Val/mean dice_metric"])
    hd95_avg.append(metric["Val/mean hd95_metric"])
    return (
...
if __name__ == "__main__":
    ...
    logging_dir = os.getcwd() + "/logs/" + current_time
    accelerator = Accelerator(
    ...
    model = SlimUNETRpp(**config.slim_unetrpp)
    ...
    plt.xlabel("epoch")
    plt.ylabel("dices_avg")
    plt.plot(trains_epochs, dices_avg, color="red")

    plt.subplot(1, 2, 2)
    plt.title("Val Mean hd95")
    plt.xlabel("epoch")
    plt.ylabel("hd95_avg")
    plt.plot(trains_epochs, hd95_avg, color="blue")
    plt.show()

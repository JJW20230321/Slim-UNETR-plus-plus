...
from src.SlimUNETRpp.Decoder import Decoder
from src.SlimUNETRpp.Encoder import Encoder


class SlimUNETRpp(nn.Module):
    def __init__(
        self,
        in_channels=4,
        out_channels=3,
        ...
        Examples:
            ...
            >>> net = SlimUNETRpp(in_channels=4, out_channels=3, embedding_dim=64)

            # for 3D single channel input with size (96, 96, 96), 2-channel output.
            >>> net = SlimUNETRpp(in_channels=1, out_channels=2, embedding_dim=27)

        """
        super(SlimUNETRpp, self).__init__()

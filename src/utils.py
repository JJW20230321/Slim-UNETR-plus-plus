...
#     def __init__(self, logdir: str):
#         self.console = sys.stdout
#         self.log_file = None  # 确保属性始终被初始化
#         if logdir is not None:
#             # 使用合法的时间戳格式，将冒号替换为下划线
#             timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
#             logdir = os.path.join(logdir, f"logs_{timestamp}")
#             try:
...
#
#     def __enter__(self):
#         return self  # 返回自身，以便使用 `with` 语句时正确初始化
#
#     def __exit__(self, *args):
#         self.close()
#
...
#
#     def close(self):
#         sys.stdout = self.console  # 恢复原来的 stdout
#         sys.stderr = self.console  # 恢复原来的 stderr
#         if self.log_file is not None:
#             self.log_file.close()
